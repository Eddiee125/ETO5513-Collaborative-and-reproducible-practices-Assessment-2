...
This section will focus on sleep efficiency using the two variables age grouped by generation and exercise frequency.
...
kable(summary_by_Exfrequency, caption = "Summary of Sleep Efficiency by Exercise Frequency") %>%
                        kable_classic(bootstrap_options = c("striped","hover"))
...
fig2_age<- ggplot(exercise_fr_percentile1, aes(freq_exercise_group, y=S_efficiency))
fig2_age + geom_line(aes(linetype = percentile, group = percentile)) +
            labs( title. = "Percentile of sleep efficiency by exercise frequency",
                          x = "Exercise Frequency Level",
                          y = "Sleep Efficiency (%)") +
                                scale_y_continuous(labels = scales::percent) +
                            theme_classic()

fig3_age<- ggplot(age_exercise_1,aes(age_group, y=Sleep.efficiency))
fig3_age + geom_point(aes(color = freq_exercise_group, group = age_group)) +
            labs( title. = "Sleep efficiency of age generations by exercise frequency",
                          x = "Generation",
                          y = "Sleep Efficiency (%)") +
                                scale_y_continuous(labels = scales::percent) +
                            theme_classic()
